...
CRamState_Patrol::CRamState_Patrol(LPDIRECT3DDEVICE9 pGraphicDev)
    : CState(pGraphicDev)
    , m_fAccTime(0.f)
    , m_fChaseRange(0.f)
...
CRamState_Patrol::~CRamState_Patrol()
{
}

HRESULT CRamState_Patrol::Ready_State(CStateMachine* pOwner)
{
    if (nullptr != pOwner)
    {
        m_pOwner = pOwner;
...
STATE_TYPE CRamState_Patrol::Update_State(const _float& fTimeDelta)
{
...
void CRamState_Patrol::LateUpdate_State()
{

}

void CRamState_Patrol::Render_State()
{
    
}

STATE_TYPE CRamState_Patrol::Key_Input(const _float& fTimeDelta)
{
 
    return m_eState;
}

CRamState_Patrol* CRamState_Patrol::Create(LPDIRECT3DDEVICE9 pGraphicDev, CStateMachine* pOwner)
{
    CRamState_Patrol* pInstance = new CRamState_Patrol(pGraphicDev);

    if (FAILED(pInstance->Ready_State(pOwner)))
    {
        Safe_Release(pInstance);
        MSG_BOX("RamState Patrol Create Failed");
        return nullptr;
