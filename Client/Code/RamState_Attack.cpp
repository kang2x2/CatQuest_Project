...
CRamState_Attack::CRamState_Attack(LPDIRECT3DDEVICE9 pGraphicDev)
    : CState(pGraphicDev)
    , m_fAccTime(0.f)
    , m_fChaseRange(0.f)
...
CRamState_Attack::~CRamState_Attack()
{
}

HRESULT CRamState_Attack::Ready_State(CStateMachine* pOwner)
{
    if (nullptr != pOwner)
    {
        m_pOwner = pOwner;
...
void CRamState_Attack::LateUpdate_State()
{

}

void CRamState_Attack::Render_State()
{
    
}

STATE_TYPE CRamState_Attack::Key_Input(const _float& fTimeDelta)
{
 
    return m_eState;
}

CRamState_Attack* CRamState_Attack::Create(LPDIRECT3DDEVICE9 pGraphicDev, CStateMachine* pOwner)
{
    CRamState_Attack* pInstance = new CRamState_Attack(pGraphicDev);
