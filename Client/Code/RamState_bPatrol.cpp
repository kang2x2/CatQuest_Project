...
CRamState_bPatrol::CRamState_bPatrol(LPDIRECT3DDEVICE9 pGraphicDev)
    : CState(pGraphicDev)
    , m_fAccTime(0.f)
    , m_fChaseRange(0.f)
...
CRamState_bPatrol::~CRamState_bPatrol()
{
}

HRESULT CRamState_bPatrol::Ready_State(CStateMachine* pOwner)
{
    if (nullptr != pOwner)
    {
        m_pOwner = pOwner;
...
STATE_TYPE CRamState_bPatrol::Update_State(const _float& fTimeDelta)
{
...
void CRamState_bPatrol::LateUpdate_State()
{

}

void CRamState_bPatrol::Render_State()
{
    
}

STATE_TYPE CRamState_bPatrol::Key_Input(const _float& fTimeDelta)
{
 
    return m_eState;
}

CRamState_bPatrol* CRamState_bPatrol::Create(LPDIRECT3DDEVICE9 pGraphicDev, CStateMachine* pOwner)
 {
    CRamState_bPatrol* pInstance = new CRamState_bPatrol(pGraphicDev);

    if (FAILED(pInstance->Ready_State(pOwner)))
    {
        Safe_Release(pInstance);
        MSG_BOX("Ram Patrol Create Failed");
        return nullptr;
