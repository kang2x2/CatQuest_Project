...
CFishState_Attack::~CFishState_Attack()
{
}

HRESULT CFishState_Attack::Ready_State(CStateMachine* pOwner)
{
    if (nullptr != pOwner)
    {
        m_pOwner = pOwner;
...
STATE_TYPE CFishState_Attack::Update_State(const _float& fTimeDelta)
{
...
void CFishState_Attack::LateUpdate_State()
{

}

void CFishState_Attack::Render_State()
{
    
}

STATE_TYPE CFishState_Attack::Key_Input(const _float& fTimeDelta)
{
 
    return m_eState;
}

CFishState_Attack* CFishState_Attack::Create(LPDIRECT3DDEVICE9 pGraphicDev, CStateMachine* pOwner)
{
    CFishState_Attack* pInstance = new CFishState_Attack(pGraphicDev);
